...
use generator_controller_lite::bps::BasicPoints;
use Event::*;
use VeEvent::*;
...
impl Simulator {
    pub fn init<T: Clone + Into<String>>(users: &[T]) -> Self {
        ...
        Self {
            helper: ControllerHelper::init(&mut router, &owner, None),
            user_votes: users
                .iter()
                .cloned()
        ...
        self.helper
            ...
            .map(|response| {
                let vp = self
                    ...
                    .unwrap();
                self.locks
                    .insert(user.to_string(), (self.router.block_period(), vp));
                ...
                assert_eq!(
                    ...
                    self.router.block_info().time.seconds()
                );
                response
            })
    }
    ...
    pub fn simulate_case(
        ...
        for period in 0..events.len() {
            ...
            for user in self.user_votes.keys() {
                ...
                if let Some((_, vp)) = self.locks.get(user) {
                    let user_vp = Uint128::from((*vp * MULTIPLIER as f32) as u128);
