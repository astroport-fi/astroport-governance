...
fn test_create_allocations() {
    ...
    assert_eq!(resp.params.unlock_schedule, None);
...
fn test_withdraw() {
    ...
    vest_resp = app
        ...
        .unwrap();
    assert_eq!(vest_resp, Uint128::from(0u64));
...
fn test_terminate() {
    ...
    let err = app
        .execute_contract(
            Addr::unchecked("not_team_1".to_string()),
            vesting_instance.clone(),
    ...
    // ######    ERROR :: No ASTRO available to refund.    ######

    app.update_block(|b| {
        b.height += 17280;
        b.time = Timestamp::from_seconds(1673938279u64)
    });

    let err = app
        .execute_contract(
            Addr::unchecked("team_1".to_string()),
            vesting_instance.clone(),
            &ExecuteMsg::Terminate {},
            &[],
        )
    ...
    assert_eq!(
        err.to_string(),
        "Generic error: No ASTRO available to refund."
    );
    ...
    let investor_vested_after: Uint128 = app
        .wrap()
        .query_wasm_smart(
            &vesting_instance,
            &QueryMsg::VestedTokens {
    ...
    let investor_alloc_after: AllocationResponse = app
        .wrap()
        .query_wasm_smart(
            &vesting_instance,
            &QueryMsg::Allocation {
